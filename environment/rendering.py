import pygame
import numpy as np
from typing import Optional, Tuple
import os
from datetime import datetime
import textwrap

class StorageVisualizer:
    """Advanced visualizer with professional styling and responsive layout"""

    # Enhanced zone colors with better contrast
    ZONE_COLORS = [
        (100, 255, 100),  # Optimal Storage (bright green)
        (100, 200, 255),  # Too Dry (light blue)
        (200, 255, 200),  # Well Ventilated (mint)
        (255, 255, 100),  # High Humidity (yellow)
        (255, 80, 80)     # Critical Risk (bright red)
    ]

    def __init__(self, env, record_gif: bool = False):
        self.env = env
        self.record_gif = record_gif
        self.frames = [] if record_gif else None
        self.agent_path = []
        self.last_direction = None
        self.animation_frame = 0
        
        pygame.init()
        try:
            # Larger window for better layout
            self.screen = pygame.display.set_mode((1000, 750))  # Increased height
            pygame.display.set_caption("Farm Storage Optimization")
        except pygame.error as e:
            raise RuntimeError(f"Display initialization failed: {str(e)}")
        
        self.clock = pygame.time.Clock()
        self._init_fonts()
        self._init_colors()
        self._calculate_layout()
        
        self.recording_dir = "recordings"
        os.makedirs(self.recording_dir, exist_ok=True)

    def _init_fonts(self):
        """Initialize comprehensive font system"""
        pygame.font.init()
        try:
            self.font_title = pygame.font.SysFont('Arial', 20, bold=True)
            self.font_large = pygame.font.SysFont('Arial', 16, bold=True)
            self.font_medium = pygame.font.SysFont('Arial', 14)
            self.font_small = pygame.font.SysFont('Arial', 12)
            self.font_tiny = pygame.font.SysFont('Arial', 10)
        except:
            # Fallback fonts
            self.font_title = pygame.font.SysFont(None, 20)
            self.font_large = pygame.font.SysFont(None, 16)
            self.font_medium = pygame.font.SysFont(None, 14)
            self.font_small = pygame.font.SysFont(None, 12)
            self.font_tiny = pygame.font.SysFont(None, 10)

    def _init_colors(self):
        """Enhanced color palette"""
        self.ui_colors = {
            'background': (245, 245, 250),
            'panel_bg': (255, 255, 255),
            'panel_border': (200, 200, 210),
            'text_primary': (40, 40, 50),
            'text_secondary': (80, 80, 90),
            'text_accent': (0, 120, 200),
            'success': (40, 180, 99),
            'warning': (243, 156, 18),
            'danger': (231, 76, 60),
            'agent_primary': (52, 152, 219),
            'agent_secondary': (241, 196, 15),
            'pest_low': (46, 204, 113),
            'pest_medium': (230, 126, 34),
            'pest_high': (231, 76, 60)
        }

    def _calculate_layout(self):
        """Calculate responsive layout dimensions with fixed spacing"""
        screen_width, screen_height = self.screen.get_size()
        
        # Title bar area (increased height)
        self.title_area = {
            'x': 0,
            'y': 0,
            'width': screen_width,
            'height': 70  # Increased from 60
        }
        
        # Legend area (positioned right below title)
        self.legend_area = {
            'x': 30,
            'y': self.title_area['height'] + 5,  # Small gap after title
            'width': screen_width - 60,
            'height': 35  # Dedicated space for legend
        }
        
        # Grid area (positioned below legend)
        self.grid_area = {
            'x': 30,
            'y': self.legend_area['y'] + self.legend_area['height'] + 10,
            'width': 500,
            'height': 500
        }
        
        # Right panel area (aligned with grid)
        self.panel_area = {
            'x': 550,
            'y': self.grid_area['y'],
            'width': 420,
            'height': 500
        }
        
        # Bottom recommendation area (positioned below grid)
        self.recommendation_area = {
            'x': 30,
            'y': self.grid_area['y'] + self.grid_area['height'] + 15,
            'width': 940,
            'height': 80
        }
        
        # Calculate grid cell size
        self.cell_size = min(
            self.grid_area['width'] // self.env.grid_size[1],
            self.grid_area['height'] // self.env.grid_size[0]
        )

    def reset(self):
        """Reset visualization state"""
        self.agent_path = []
        self.last_direction = None
        self.animation_frame = 0
        if self.record_gif:
            self.frames = []

    def render(self) -> Optional[np.ndarray]:
        """Main rendering with advanced features"""
        try:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    raise KeyboardInterrupt("Window closed")

            self.animation_frame += 1
            self.screen.fill(self.ui_colors['background'])
            
            # Draw components in proper order with fixed layout
            self._draw_title_bar()
            self._draw_legend()  # Draw legend after title but before grid
            self._draw_zone_grid()
            self._draw_agent_with_effects()
            self._draw_information_panel()
            self._draw_recommendation_panel()
            
            pygame.display.flip()
            self.clock.tick(30)  # Smoother animation
            
            if self.record_gif:
                self._capture_frame()
            
            return self._get_rgb_array()
        
        except Exception as e:
            print(f"Rendering error: {str(e)}")
            return None

    def _draw_title_bar(self):
        """Professional title bar with key metrics - fixed height"""
        title_area = self.title_area
        
        # Background bar
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_bg'],
            (title_area['x'], title_area['y'], title_area['width'], title_area['height']),
            border_radius=0
        )
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_border'],
            (0, title_area['height'] - 2, title_area['width'], 2)
        )
        
        # Title and key info
        title = f"Farm Storage Optimization - Day {self.env.day}/{self.env.config['max_days']}"
        title_surface = self.font_title.render(title, True, self.ui_colors['text_primary'])
        self.screen.blit(title_surface, (20, 15))
        
        # Key metrics on the right - positioned better
        pest_color = self._get_pest_color(self.env.pest_level)
        metrics = [
            f"Position: [{self.env.current_pos[0]}, {self.env.current_pos[1]}]",
            f"Pest: {self.env.pest_level*100:.1f}%",
            f"Temp: {self.env.state['temp'][0]:.1f}°C",  
            f"Reward: {self.env.cumulative_reward:.1f}"
        ]
        
        x_offset = self.screen.get_width() - 420
        for i, metric in enumerate(metrics):
            color = pest_color if "Pest:" in metric else self.ui_colors['text_secondary']
            metric_surface = self.font_medium.render(metric, True, color)
            self.screen.blit(metric_surface, (x_offset + i * 105, 25))

    def _draw_legend(self):
        """Complete horizontal legend with all zone types"""
        legend_area = self.legend_area
        
        # Legend background
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_bg'],
            (legend_area['x'], legend_area['y'], legend_area['width'], legend_area['height']),
            border_radius=8
        )
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_border'],
            (legend_area['x'], legend_area['y'], legend_area['width'], legend_area['height']),
            1, border_radius=8
        )
        
        # Legend title
        legend_title = self.font_small.render("Zone Types:", True, self.ui_colors['text_accent'])
        self.screen.blit(legend_title, (legend_area['x'] + 10, legend_area['y'] + 5))
        
        # All zone types with proper spacing
        x_offset = legend_area['x'] + 100
        zone_spacing = 150  # Fixed spacing between legend items
        
        for i, zone_type in enumerate([0, 1, 2, 3, 4]):  # Include ALL zone types including "Too Dry"
            zone = self.env.STATE_TYPES[zone_type]
            
            # Color swatch
            swatch_x = x_offset + i * zone_spacing
            swatch_y = legend_area['y'] + 8
            
            pygame.draw.rect(
                self.screen,
                self.ZONE_COLORS[zone_type],
                (swatch_x, swatch_y, 18, 18),
                border_radius=4
            )
            pygame.draw.rect(
                self.screen,
                self.ui_colors['panel_border'],
                (swatch_x, swatch_y, 18, 18),
                1, border_radius=4
            )
            
            # Label with shortened names to fit better
            short_names = {
                "Optimal Storage": "Optimal",
                "Too Dry": "Too Dry", 
                "Well Ventilated": "Ventilated",
                "High Humidity": "High Humid",
                "Critical Risk": "Critical"
            }
            
            label_text = short_names.get(zone["name"], zone["name"])
            label_surface = self.font_tiny.render(label_text, True, self.ui_colors['text_secondary'])
            self.screen.blit(label_surface, (swatch_x + 25, swatch_y + 2))

    def _draw_zone_grid(self):
        """Enhanced grid with shadows and hover effects"""
        grid_x, grid_y = self.grid_area['x'], self.grid_area['y']
        
        # Draw shadow first
        shadow_offset = 3
        pygame.draw.rect(
            self.screen,
            (0, 0, 0, 30),
            (grid_x + shadow_offset, grid_y + shadow_offset, 
             self.env.grid_size[1] * self.cell_size, 
             self.env.grid_size[0] * self.cell_size),
            border_radius=8
        )
        
        for row in range(self.env.grid_size[0]):
            for col in range(self.env.grid_size[1]):
                zone_type = self.env.grid_states[row, col]
                x = grid_x + col * self.cell_size
                y = grid_y + row * self.cell_size
                
                # Zone background with gradient effect
                color = self.ZONE_COLORS[zone_type]
                
                # Create gradient effect for better depth
                for i in range(3):
                    shade = tuple(max(0, c - i * 10) for c in color)
                    pygame.draw.rect(
                        self.screen,
                        shade,
                        (x + i, y + i, self.cell_size - 2*i, self.cell_size - 2*i),
                        border_radius=6 - i
                    )
                
                # Zone label with better contrast
                zone_name = self.env.STATE_TYPES[zone_type]["name"]
                short_name = self._get_short_zone_name(zone_name)
                
                # Text background for readability
                text_surface = self.font_small.render(short_name, True, (255, 255, 255))
                text_bg = pygame.Surface((text_surface.get_width() + 6, text_surface.get_height() + 4))
                text_bg.fill((0, 0, 0))
                text_bg.set_alpha(120)
                
                text_x = x + self.cell_size//2 - text_surface.get_width()//2
                text_y = y + self.cell_size//2 - text_surface.get_height()//2
                
                self.screen.blit(text_bg, (text_x - 3, text_y - 2))
                self.screen.blit(text_surface, (text_x, text_y))
                
                # Border
                pygame.draw.rect(
                    self.screen,
                    self.ui_colors['panel_border'],
                    (x, y, self.cell_size, self.cell_size),
                    2, border_radius=6
                )

    def _get_short_zone_name(self, name):
        """Get shortened zone names for grid display"""
        name_map = {
            "Optimal Storage": "Opt",
            "Too Dry": "Dry",
            "Well Ventilated": "Vent",
            "High Humidity": "Humid",
            "Critical Risk": "RISK"
        }
        return name_map.get(name, name[:4])

    def _draw_agent_with_effects(self):
        """Advanced agent rendering with animations"""
        grid_x, grid_y = self.grid_area['x'], self.grid_area['y']
        
        # Current position center
        agent_x = grid_x + self.env.current_pos[1] * self.cell_size + self.cell_size//2
        agent_y = grid_y + self.env.current_pos[0] * self.cell_size + self.cell_size//2
        
        # Pulsing effect based on animation frame
        pulse = abs(np.sin(self.animation_frame * 0.2)) * 3
        
        # Agent shadow
        pygame.draw.circle(
            self.screen,
            (0, 0, 0, 50),
            (agent_x + 2, agent_y + 2),
            int(15 + pulse)
        )
        
        # Agent body with gradient
        for i in range(3):
            radius = int(15 + pulse - i * 2)
            color_intensity = 255 - i * 40
            agent_color = (
                min(255, self.ui_colors['agent_primary'][0] + color_intensity - 200),
                min(255, self.ui_colors['agent_primary'][1] + color_intensity - 200),
                min(255, self.ui_colors['agent_primary'][2] + color_intensity - 100)
            )
            pygame.draw.circle(self.screen, agent_color, (agent_x, agent_y), radius)
        
        # Direction indicator with smooth animation
        if self.last_direction and (self.last_direction[0] != 0 or self.last_direction[1] != 0):
            dx, dy = self.last_direction
            arrow_length = 20
            
            # Calculate arrow direction
            if dx > 0:  # Right
                arrow_points = [
                    (agent_x + arrow_length, agent_y),
                    (agent_x + arrow_length - 8, agent_y - 6),
                    (agent_x + arrow_length - 8, agent_y + 6)
                ]
            elif dx < 0:  # Left
                arrow_points = [
                    (agent_x - arrow_length, agent_y),
                    (agent_x - arrow_length + 8, agent_y - 6),
                    (agent_x - arrow_length + 8, agent_y + 6)
                ]
            elif dy > 0:  # Down
                arrow_points = [
                    (agent_x, agent_y + arrow_length),
                    (agent_x - 6, agent_y + arrow_length - 8),
                    (agent_x + 6, agent_y + arrow_length - 8)
                ]
            else:  # Up
                arrow_points = [
                    (agent_x, agent_y - arrow_length),
                    (agent_x - 6, agent_y - arrow_length + 8),
                    (agent_x + 6, agent_y - arrow_length + 8)
                ]
            
            pygame.draw.polygon(self.screen, self.ui_colors['agent_secondary'], arrow_points)

    def _draw_information_panel(self):
        """Professional information panel"""
        panel_x, panel_y = self.panel_area['x'], self.panel_area['y']
        panel_w, panel_h = self.panel_area['width'], self.panel_area['height']
        
        # Panel background with shadow
        pygame.draw.rect(
            self.screen,
            (0, 0, 0, 20),
            (panel_x + 3, panel_y + 3, panel_w, panel_h),
            border_radius=12
        )
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_bg'],
            (panel_x, panel_y, panel_w, panel_h),
            border_radius=12
        )
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_border'],
            (panel_x, panel_y, panel_w, panel_h),
            2, border_radius=12
        )
        
        # Current zone info
        zone_type = self.env.state["zone_type"]
        zone = self.env.STATE_TYPES[zone_type]
        
        y_pos = panel_y + 20
        
        # Zone header
        zone_title = self.font_large.render(f"Current Zone: {zone['name']}", True, self.ui_colors['text_primary'])
        self.screen.blit(zone_title, (panel_x + 20, y_pos))
        y_pos += 35
        
        # Zone description with word wrapping
        description_lines = textwrap.wrap(zone['description'], width=50)
        for line in description_lines:
            desc_surface = self.font_small.render(line, True, self.ui_colors['text_secondary'])
            self.screen.blit(desc_surface, (panel_x + 20, y_pos))
            y_pos += 20
        
        y_pos += 10
        
        # Environmental conditions
        self._draw_info_section("Environmental Conditions", panel_x + 20, y_pos)
        y_pos += 30
        
        info_data = [
            ("Temperature", f"{self.env.state['temp'][0]:.1f}°C", self._get_temp_color(self.env.state['temp'][0])),
            ("Humidity", f"{self.env.state['humidity'][0]:.1f}%", self._get_humidity_color(self.env.state['humidity'][0])),
            ("Crop Type", self.env.CROP_TYPES[self.env.state['crop_type']], self.ui_colors['text_secondary']),
            ("Storage Method", self.env.STORAGE_METHODS[self.env.state['storage_method']], self.ui_colors['text_secondary']),
        ]
        
        for label, value, color in info_data:
            label_surface = self.font_medium.render(f"{label}:", True, self.ui_colors['text_secondary'])
            value_surface = self.font_medium.render(value, True, color)
            self.screen.blit(label_surface, (panel_x + 30, y_pos))
            self.screen.blit(value_surface, (panel_x + 200, y_pos))
            y_pos += 25
        
        y_pos += 10
        
        # Progress indicators
        self._draw_info_section("Progress", panel_x + 20, y_pos)
        y_pos += 35
        
        # Pest risk meter with gradient
        self._draw_pest_meter(panel_x + 30, y_pos, panel_w - 60)
        y_pos += 50
        
        # Suitability score
        suitability = self.env.calculate_zone_suitability()[tuple(self.env.current_pos)]
        self._draw_progress_bar("Zone Suitability", suitability, panel_x + 30, y_pos, panel_w - 60, self.ui_colors['success'])

    def _draw_info_section(self, title, x, y):
        """Draw section header"""
        title_surface = self.font_large.render(title, True, self.ui_colors['text_accent'])
        self.screen.blit(title_surface, (x, y))
        # Underline
        pygame.draw.line(
            self.screen,
            self.ui_colors['text_accent'],
            (x, y + 22),
            (x + title_surface.get_width(), y + 22),
            2
        )

    def _draw_pest_meter(self, x, y, width):
        """Advanced pest risk meter with color gradient"""
        # Background
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_border'],
            (x, y, width, 25),
            border_radius=12
        )
        
        # Fill based on pest level
        fill_width = int(width * self.env.pest_level)
        pest_color = self._get_pest_color(self.env.pest_level)
        
        if fill_width > 0:
            pygame.draw.rect(
                self.screen,
                pest_color,
                (x, y, fill_width, 25),
                border_radius=12
            )
        
        # Text overlay
        pest_text = f"Pest Risk: {self.env.pest_level*100:.1f}%"
        text_color = (255, 255, 255) if self.env.pest_level > 0.5 else self.ui_colors['text_primary']
        text_surface = self.font_medium.render(pest_text, True, text_color)
        text_x = x + width//2 - text_surface.get_width()//2
        self.screen.blit(text_surface, (text_x, y + 5))

    def _draw_progress_bar(self, label, value, x, y, width, color):
        """Generic progress bar"""
        # Label
        label_surface = self.font_medium.render(f"{label}: {value:.1%}", True, self.ui_colors['text_secondary'])
        self.screen.blit(label_surface, (x, y))
        
        # Bar background
        bar_y = y + 20
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_border'],
            (x, bar_y, width, 15),
            border_radius=8
        )
        
        # Fill
        fill_width = int(width * max(0, min(1, value)))
        if fill_width > 0:
            pygame.draw.rect(
                self.screen,
                color,
                (x, bar_y, fill_width, 15),
                border_radius=8
            )

    def _draw_recommendation_panel(self):
        """Fixed recommendation panel that doesn't overflow"""
        rec_x, rec_y = self.recommendation_area['x'], self.recommendation_area['y']
        rec_w, rec_h = self.recommendation_area['width'], self.recommendation_area['height']
        
        # Panel background
        pygame.draw.rect(
            self.screen,
            self.ui_colors['panel_bg'],
            (rec_x, rec_y, rec_w, rec_h),
            border_radius=8
        )
        pygame.draw.rect(
            self.screen,
            self.ui_colors['success'],
            (rec_x, rec_y, rec_w, rec_h),
            2, border_radius=8
        )
        
        # Get recommendations
        zone_type = self.env.state["zone_type"]
        zone_info = self.env.STATE_TYPES[zone_type]
        treatment_actions = [a for a in zone_info["actions"] if a <= 16]
        
        # Title
        title_surface = self.font_large.render("Recommended Actions:", True, self.ui_colors['success'])
        self.screen.blit(title_surface, (rec_x + 15, rec_y + 10))
        
        # Actions with proper text wrapping
        if treatment_actions:
            action_names = [self.env.ACTIONS[a] for a in treatment_actions if a < len(self.env.ACTIONS)]
            action_text = " • ".join(action_names)
            
            # Wrap text to fit in the recommendation area
            max_chars_per_line = (rec_w - 30) // 8  # Approximate character width
            wrapped_lines = textwrap.wrap(action_text, width=max_chars_per_line)
            
            y_offset = rec_y + 35
            for line in wrapped_lines[:2]:  # Maximum 2 lines
                line_surface = self.font_medium.render(line, True, self.ui_colors['text_primary'])
                self.screen.blit(line_surface, (rec_x + 15, y_offset))
                y_offset += 20
        else:
            warning_surface = self.font_medium.render("⚠️ No valid actions available!", True, self.ui_colors['danger'])
            self.screen.blit(warning_surface, (rec_x + 15, rec_y + 35))

    def _get_pest_color(self, pest_level):
        """Get color based on pest level"""
        if pest_level < 0.3:
            return self.ui_colors['pest_low']
        elif pest_level < 0.7:
            return self.ui_colors['pest_medium']
        else:
            return self.ui_colors['pest_high']

    def _get_temp_color(self, temp):
        """Get color based on temperature"""
        if 18 <= temp <= 25:
            return self.ui_colors['success']
        elif 15 <= temp <= 30:
            return self.ui_colors['warning']
        else:
            return self.ui_colors['danger']

    def _get_humidity_color(self, humidity):
        """Get color based on humidity"""
        if 55 <= humidity <= 70:
            return self.ui_colors['success']
        elif 45 <= humidity <= 80:
            return self.ui_colors['warning']
        else:
            return self.ui_colors['danger']

    def _capture_frame(self):
        """Capture frame for GIF recording"""
        rgb_array = self._get_rgb_array()
        if rgb_array is not None:
            self.frames.append(rgb_array)

    def _get_rgb_array(self) -> Optional[np.ndarray]:
        """Convert screen to RGB array"""
        try:
            return np.transpose(
                pygame.surfarray.array3d(self.screen),
                axes=(1, 0, 2)
            )
        except:
            return None

    def save_gif(self, filename: Optional[str] = None):
        """Save recorded frames as GIF"""
        if not self.frames or not self.record_gif:
            return
            
        try:
            import imageio
            filename = filename or f"{self.recording_dir}/sim_{datetime.now().strftime('%Y%m%d_%H%M%S')}.gif"
            imageio.mimsave(filename, self.frames, fps=10)
            print(f"Saved GIF: {filename}")
        except ImportError:
            print("GIF saving requires imageio package")

    def close(self):
        """Clean up resources"""
        if self.record_gif and self.frames:
            self.save_gif()
        pygame.quit()