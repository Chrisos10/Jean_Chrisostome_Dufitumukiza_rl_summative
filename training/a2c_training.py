...
class A2CMetricsCallback(BaseCallback):
    """ Callback class to collect A2C training metrics and save as CSV files"""
    ...
    def _on_rollout_end(self) -> bool:
        ...
        # collecting training metrics from A2C logger
        ...
        for i, (episode_reward, episode_length) in enumerate(zip(self.temp_episode_rewards, self.temp_episode_lengths)):
            # Update counters
            ...
            episode_record = {
                ...
                'zones_reached': False,
                'correct_zones': False,
                'treatments_applied': False,
                'episode_phases': 'UNKNOWN',
                'final_distances': 0,
                'best_distances': 0,
    ...
    def save_metrics_csv(self, filename_suffix=""):
        ...
        if self.episode_data:
            ...
            episode_file = f"episode_metrics_{filename_suffix}_{timestamp}.csv"
            ...
            # Also saving the key metrics + training metrics
            simple_episode_df = episode_df[['episode_num', 'episode_reward', 'cumulative_reward', 
                                          ...
                                          'explained_variance', 'total_loss', 'loss_variance_window']].copy()
            simple_file = f"episode_rewards_{filename_suffix}_{timestamp}.csv"
            ...
            print(f"Simplified episode rewards saved to: {simple_path}")
        
        # Save training metrics
        if self.training_data:
            training_df = pd.DataFrame(self.training_data)
            training_file = f"training_metrics_{filename_suffix}_{timestamp}.csv"
            training_path = os.path.join(self.save_path, training_file)
            training_df.to_csv(training_path, index=False)
            saved_files['training_metrics'] = training_path
            print(f"Training metrics saved to: {training_path}")
        ...
        if self.episode_data:
            ...
            summary_file = f"training_summary_{filename_suffix}_{timestamp}.csv"
...
class CustomMonitor(Monitor):
    ...
    def step(self, action):
        ...
        # Update episode statistics (ensuring reward is treated as a scalar)
...
def train_a2c(total_timesteps=100000, config=None, save_path="./models/a2c/"):
    """Train A2C agent on storage environment"""
    ...
    callback = A2CMetricsCallback(save_path=log_path, save_freq=5000, verbose=1)
    
    # A2C Hyperparameters (FIXED for SB3 v1.8.0+)
    a2c_params = {
        'learning_rate': 5e-4,
        'gamma': 0.99,
        'gae_lambda': 0.95,
    ...
    for key, value in a2c_params.items():
        if key != 'policy_kwargs':
            print(f"  {key}: {value}")
    print(f"  policy_network: {a2c_params['policy_kwargs']['net_arch']['pi']}")
    print(f"  value_network: {a2c_params['policy_kwargs']['net_arch']['vf']}")
    print(f"  activation_function: {a2c_params['policy_kwargs']['activation_fn'].__name__}")
    print(f"  device: {a2c_params['device']}")
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    model_path = os.path.join(save_path, f"a2c_storage_model_{timestamp}")
    model.save(model_path)
    
    # Save hyperparameters
    hyperparams_path = os.path.join(save_path, f"a2c_hyperparams_{timestamp}.json")
    ...
    if callback.episode_data:
        ...
        if callback.training_data and callback.training_data[-1]['policy_loss'] is not None:
            print(f"  Final policy loss: {callback.training_data[-1]['policy_loss']:.4f}")
        if callback.training_data and callback.training_data[-1]['entropy'] is not None:
            print(f"  Final entropy: {callback.training_data[-1]['entropy']:.4f}")
        if callback.training_data and callback.training_data[-1]['explained_variance'] is not None:
...
if __name__ == "__main__":
    ...
    # Train A2C
    model, callback, model_path = train_a2c(
